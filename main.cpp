...
Lista<Album> cargarAlbumesYCanciones(const string& archivoAlbumes, const string& archivoCanciones) {
    ...
    while (getline(archivoAlbums, lineaAlbum)) {
	    ...
	    //Arreglo para que la información quede guardada en las cabeceras
...
//Por la naturaleza de un par de consultas necesité hacer una lista de todas las canciones, esto crea una lista de apuntadores a las canciones que están guardads en 
//La lista de Albumes, y'know, para asegurar atomicidad de información
...
int main() {
	Cola<Artista> cola;
	//Creación de las listas generales de Albumes y Canciones
	...
	//Ejemplo de visualización de la información contenida en un album de la lista
